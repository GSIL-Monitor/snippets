...
int main(int argc, char** argv)
{
	...
	if (!socket) {
		fprintf(stderr, "%s", "amqp_tcp_socket_new error\n");
		exit(1);
	}

	status = amqp_socket_open(socket, hostname, port);
	if (status) {
		fprintf(stderr, "%s", "amqp_socket_open error\n");
		exit(1);
	}

	die_on_amqp_error(
		amqp_login(
			conn, "/", 0, 131072, 0, AMQP_SASL_METHOD_PLAIN, "guest", "guest"),
		"Logging in");
	...
	amqp_basic_consume(
		conn, 1, queuename, amqp_empty_bytes, 0, 1, 0, amqp_empty_table);
	...
	die_on_error(
		amqp_destroy_connection(conn), "Endin connection");
