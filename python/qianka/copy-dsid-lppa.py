...
class Converter(object):
    ...
    def queryNew(self, rowIds):
        if len(rowIds) <= 0:
            return set()
        sql = """SELECT id FROM dsid_lppa_v3
        WHERE id IN :rowIds"""
        p = {
            'rowIds': rowIds,
        }
    ...
    def copyRow(self, row):
        ...
        # sql = """INSERT INTO dsid_lppa_v3 (id, dsid, lppa)
        # VALUES (:rowId, :dsid, :blob) ON DUPLICATE KEY UPDATE id = id"""
        # p = {
        #     'blob': blob,
        #     'rowId': row['id'],
        #     'dsid': row['dsid'],
