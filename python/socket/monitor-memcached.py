...
ap.add_argument('-m', '--memcached',
                help='memcached server host[:port]', required=True)
ap.add_argument('-n', '--name', help='node name, example: n1368', required=True)
ap.add_argument('-s', '--statsd',
                help='statsd server host[:port]')
ap.add_argument('-d', '--dry', help='dry run, show detail, not sending metrics',
                action='store_true')
...
def monitor():
    ...
    if total > 0:
        print('hit rate: ' + '%.2f' % ((hits * 100) / total) + '%')
        print('miss rate: ' + '%.2f' % ((misses * 100) / total) + '%')
